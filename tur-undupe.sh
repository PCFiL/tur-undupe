...
if [ -z "$filename" ]; then
  echo -e "$header \002$A1\002 was not found in dupefile."
  if [ -z "`$dupelist | head -n1`" ]; then
    echo -e "$header In fact, it dosnt seem like anything is duped from running dupelist. You might need to recompile it."
  fi
  exit 0
else
  if [ "$filetime" != "0" ]; then
    ...
    datum_1="$filetime"
    proc_calctime
    echo -en "$header Unduping \002$A1\002, uploaded by $filewho - $timeago ago"
  else
  ...
  if [ "$filename" ]; then
    if [ "$NOCHECK" = "TRUE" ]; then
      echo -e " - \002FAILED!\002 - Set NOCHECK to FALSE to see why."
    else
      echo -e " - \002FAILED\002"
    fi
  else
    echo -e " - \002OK!\002"
  fi
